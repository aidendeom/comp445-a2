...
Router::Router(char *fn)		//Constructor
{
	...
	try
	{
		...
		sa_in.sin_port = htons(ROUTER_PORT_CLIENT);
		...
		//Fill-in UDP Port and Address info.
		sa_in.sin_family = AF_INET;
		sa_in.sin_port = htons(ROUTER_PORT_SERVER);
		sa_in.sin_addr.s_addr = htonl(INADDR_ANY);
		...
		cout << "\nPlease enter the first peer host name:" << flush;
		cin >> peer_name1;
		cout << "\nPlease enter the second peer host name:" << flush;
		cin >> peer_name2;
		cout << "\nPlease enter the drop rate:" << flush;
		cin >> damage_rate;
		cout << "\nPlease enter the delay rate:" << flush;
		cin >> delay_rate;
		...
		sa_in_peer1.sin_port = htons(PEER_PORT_CLIENT);
		...
		sa_in_peer2.sin_port = htons(PEER_PORT_SERVER);
